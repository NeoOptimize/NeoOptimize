...
function Start-NeoVoiceAction {
    ...
    $args = "-NoProfile -ExecutionPolicy Bypass -File $(Quote-Arg $EnginePath) -Action $Action"
    Start-Process -FilePath (Get-PowerShellExe) -ArgumentList $args -WorkingDirectory $Root -WindowStyle Hidden | Out-Null
}

function Complete-VoiceCommand {
    param(
        [bool]$Ok,
        [string]$Message,
        [string]$Transcript = "",
    ...
    if (-not [string]::IsNullOrWhiteSpace($Action)) {
        Write-Host "Action       : $Label [$Action]" -ForegroundColor Cyan
    }
    if ($Ok) {
...
try {
    ...
    if (-not $recognized) {
        Complete-VoiceCommand -Ok $false -Message "No clear voice command recognized. Try: '$wake doctor', '$wake clean', '$wake network', or type in NEO Mini." -ExitCode 2
    }

    $match = Get-NeoVoiceAction -Text $recognized.Text
    if (-not $match) {
        Complete-VoiceCommand -Ok $false -Message "Voice was recognized, but no NeoOptimize action matched this phrase." -Transcript $recognized.Text -ExitCode 3
    }

    if (-not $NoExecute) {
        Start-NeoVoiceAction -Action $match.action
    }
    Complete-VoiceCommand -Ok $true -Message ("Voice command recognized with {0}% confidence." -f ([math]::Round($recognized.Confidence * 100))) -Transcript $recognized.Text -Action $match.action -Label $match.label
} catch {
    Complete-VoiceCommand -Ok $false -Message "Voice command is unavailable: $($_.Exception.Message). Install a Windows speech language pack and make sure a microphone is available." -ExitCode 1
}
