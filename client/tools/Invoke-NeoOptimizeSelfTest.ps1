...
$required = @(
    ...
    "config\NeoOptimize.OperatorPolicy.json",
    ...
    "tools\Enable-NeoOptimizeRemoteAccess.ps1",
    "tools\nullclaw.ps1",
    "tools\nullclaw.cmd",
    "tools\neo_mcp_bridge.ps1",
    "skills\neo-windows-doctor.json",
    "skills\neo-computer-use.json",
    "skills\neo-optional-tooling.json",
    "mcp\neo-mcp-bridge.json",
    "lib\Common.ps1",
...
$corpusPath = Join-Path $root "knowledge\neo-ai-corpus.jsonl"
$corpusManifestPath = Join-Path $root "knowledge\neo-ai-corpus.manifest.json"
if ((Test-Path $corpusPath) -and (Test-Path $corpusManifestPath)) {
    try {
        $manifest = Get-Content -Path $corpusManifestPath -Raw | ConvertFrom-Json
        $sha = (Get-FileHash -Path $corpusPath -Algorithm SHA256).Hash.ToLowerInvariant()
        if ([string]$manifest.corpus_sha256 -ne $sha) {
            Add-Failure "NEO corpus SHA-256 mismatch. Expected $($manifest.corpus_sha256), got $sha"
        }
        if ([int]$manifest.record_count -lt 100) {
            Add-Failure "NEO corpus record count is too low: $($manifest.record_count)"
        }
    } catch {
        Add-Failure "NEO corpus manifest/hash validation failed: $($_.Exception.Message)"
    }
}
...
$dangerChecks = @(
    ...
    @{
        File = "modules\03_Privacy.ps1"
        Pattern = 'Set-Reg\s+\$privPath\s+["'']LetAppsAccess(?:Camera|Microphone|Location)["'']\s+2'
        Message = "Privacy must not lock Camera/Microphone/Location through organization AppPrivacy policy."
    },
    @{
        File = "modules\03_Privacy.ps1"
        Pattern = 'Set-Reg\s+\$locPath\s+["'']Disable(?:Location|LocationScripting|Sensors)["'']\s+1'
        Message = "Privacy must not disable Location/Sensors through organization policy."
    },
    @{
