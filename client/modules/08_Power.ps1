...
if (-not (Test-NeoHighRiskConsent -ActionName "Power & Gaming Mode" -RiskLevel "High" -Reason "Dapat mengubah power plan, hibernate, BCD timer flags, GPU scheduling, dan latency tweaks.")) {
    Wait-AnyKey
    return
}
...
Write-Host "  $($Global:CYAN)[3]$($Global:RESET)  NEOOPTIMIZE MAX PERFORMANCE   Custom plan: CPU 100%, hibernate off"
Write-Host "  $($Global:CYAN)[4]$($Global:RESET)   BALANCED               Windows default"
...
switch ($choice) {
    ...
    "3" { # CUSTOM MAX PERFORMANCE
        Write-Host ""
        Write-Step "NEOOPTIMIZE MAX PERFORMANCE  CUSTOM PLAN"
        Write-Host ""
        $raw = powercfg /duplicatescheme $GUID_HIGH 2>&1
        $gm  = if ($raw -match "\{([0-9a-f-]+)\}") { $Matches[1] } else { $GUID_HIGH }

        powercfg /changename $gm "NeoOptimize Max Performance" "Custom: CPU 100%, Zero Sleep, Low Latency" 2>&1 | Out-Null
        ...
        Write-OK "NeoOptimize Max Performance plan aktif"
