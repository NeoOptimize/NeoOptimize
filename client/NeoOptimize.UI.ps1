...
if (-not (Test-AdminSession)) {
    ...
    Start-Process -FilePath $ps -ArgumentList "-Sta -NoProfile -ExecutionPolicy RemoteSigned -File `"$PSCommandPath`"" -Verb RunAs
    exit
}

foreach ($dir in @($Script:ReportsPath, (Split-Path -Parent $Script:ConfigPath))) {
...
function Read-UiConfig {
    $default = [PSCustomObject]@{ language = "en"; theme = "dark" }
...
$Script:L = @{
    en = @{
        ...
        Operations = @(
            ...
            @{ Badge="SEC"; Name="Security"; Risk="Medium"; Action="Security"; Detail="Defender, firewall, SMB, TLS, UAC, exploit protection." },
...
$Script:OverviewModules = @(
    ...
    @{ Icon="🛡"; Color="#FF3366"; Name="Security Hardening";    Action="Security";          Detail="Tighten Defender, firewall, SMB, TLS, and exploit protection." },
...
$Script:ModuleGroups = @{
    Diagnostics = @("Dashboard", "AIPlan", "AIInteractive", "AIScriptForge", "VoiceCommand", "DeepScan", "SystemDiagnostics", "WindowsDoctor", "DiskStatus", "DiskScan", "AgentAudit")
    Security    = @("Security", "Privacy", "Network", "Services")
    Optimize    = @("SmartOptimize", "Cleaner", "CleanAll", "ScheduleClean", "Performance", "Power", "Updates", "NeoUpdate")
    Disk        = @("DiskStatus", "DiskScan", "DiskRepair", "DiskOptimize", "HealthRepair", "WindowsErrorFix", "SystemRepair")
}
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="NeoOptimize Control Center" Width="1080" Height="680" MinWidth="980" MinHeight="620"
  ...
  <Grid Background="{DynamicResource PageBrush}">
    ...
    <Border Grid.Column="0" Background="{DynamicResource SidebarBrush}" BorderBrush="{DynamicResource LineBrush}" BorderThickness="0,0,1,0">
      <Grid Margin="18">
        ...
        <StackPanel Grid.Row="2">
          <TextBlock Text="MONITOR" Foreground="{DynamicResource MutedBrush}" FontSize="10" FontWeight="SemiBold" Margin="4,0,0,6"/>
          <Button x:Name="BtnOverview" Style="{StaticResource NavButton}" Margin="0,0,0,6"/>
          <Button x:Name="BtnAdvisor" Style="{StaticResource NavButton}" Margin="0,0,0,6"/>
          <TextBlock Text="SECURITY" Foreground="{DynamicResource MutedBrush}" FontSize="10" FontWeight="SemiBold" Margin="4,6,0,6"/>
          <Button x:Name="BtnProviders" Style="{StaticResource NavButton}" Margin="0,0,0,6"/>
          <Button x:Name="BtnAudit" Style="{StaticResource NavButton}" Margin="0,0,0,6"/>
          <TextBlock Text="OPTIMIZE" Foreground="{DynamicResource MutedBrush}" FontSize="10" FontWeight="SemiBold" Margin="4,6,0,6"/>
          <Button x:Name="BtnRestore" Style="{StaticResource NavButton}" Margin="0,0,0,6"/>
          <Button x:Name="BtnReports" Style="{StaticResource NavButton}" Margin="0,0,0,6"/>
          <TextBlock Text="MANAGE" Foreground="{DynamicResource MutedBrush}" FontSize="10" FontWeight="SemiBold" Margin="4,6,0,6"/>
          <Button x:Name="BtnConsole" Style="{StaticResource NavButton}" Margin="0,0,0,6"/>
          <Button x:Name="BtnUsers" Style="{StaticResource NavButton}" Margin="0,0,0,6"/>
          <Button x:Name="BtnSettings" Style="{StaticResource NavButton}" Margin="0,0,0,6"/>
          <Button x:Name="BtnAbout" Style="{StaticResource NavButton}" Margin="0,0,0,6"/>
        </StackPanel>
    ...
    <Grid Grid.Column="1" Margin="18">
      ...
      <Grid Grid.Row="2">
        <Grid x:Name="PageOverview">
          ...
          <Grid Grid.Row="1" Grid.ColumnSpan="2">
            ...
            <Border Background="{DynamicResource PanelBrush}" BorderBrush="{DynamicResource LineBrush}" BorderThickness="1" CornerRadius="12" Padding="12" Margin="0,0,10,0">
              <StackPanel>
                <DockPanel Margin="0,0,0,8">
                  <TextBlock Text="CPU Usage" Foreground="{DynamicResource TextBrush}" FontSize="12" FontWeight="SemiBold" DockPanel.Dock="Left"/>
                  <TextBlock Text="LIVE" Foreground="{DynamicResource AccentBrush}" FontSize="10" FontWeight="Bold" DockPanel.Dock="Right"/>
                </DockPanel>
                <Canvas x:Name="CpuChartCanvas" Height="132" Background="{DynamicResource PanelAltBrush}" />
              </StackPanel>
            </Border>

            <Border Grid.Column="1" Background="{DynamicResource PanelBrush}" BorderBrush="{DynamicResource LineBrush}" BorderThickness="1" CornerRadius="12" Padding="12" Margin="0,0,10,0">
              <StackPanel>
                <DockPanel Margin="0,0,0,8">
                  <TextBlock Text="Memory Usage" Foreground="{DynamicResource TextBrush}" FontSize="12" FontWeight="SemiBold" DockPanel.Dock="Left"/>
                  <TextBlock Text="LIVE" Foreground="{DynamicResource AccentBrush}" FontSize="10" FontWeight="Bold" DockPanel.Dock="Right"/>
                </DockPanel>
                <Canvas x:Name="RamChartCanvas" Height="132" Background="{DynamicResource PanelAltBrush}" />
              </StackPanel>
            </Border>

            <Border Grid.Column="2" Background="{DynamicResource PanelBrush}" BorderBrush="{DynamicResource LineBrush}" BorderThickness="1" CornerRadius="12" Padding="12">
              <StackPanel>
                <DockPanel Margin="0,0,0,8">
                  <TextBlock Text="Network Throughput" Foreground="{DynamicResource TextBrush}" FontSize="12" FontWeight="SemiBold" DockPanel.Dock="Left"/>
                  <TextBlock Text="LIVE" Foreground="{DynamicResource AccentBrush}" FontSize="10" FontWeight="Bold" DockPanel.Dock="Right"/>
                </DockPanel>
                <Canvas x:Name="NetworkChartCanvas" Height="132" Background="{DynamicResource PanelAltBrush}" />
              </StackPanel>
            </Border>
          </Grid>
        </Grid>
        ...
        <Grid x:Name="PageAi" Visibility="Collapsed">
          ...
          <Border Grid.Column="1" Background="{DynamicResource PanelBrush}" BorderBrush="{DynamicResource LineBrush}" BorderThickness="1" CornerRadius="12" Padding="16">
            <StackPanel>
              <TextBlock x:Name="CloudTitleText" Foreground="{DynamicResource TextBrush}" FontSize="18" FontWeight="SemiBold"/>
              <TextBlock x:Name="CloudNoteText" Foreground="{DynamicResource MutedBrush}" FontSize="12" TextWrapping="Wrap" Margin="0,6,0,14"/>
              <StackPanel x:Name="ConnectorRows" Margin="0,0,0,14"/>
              <Button x:Name="BtnCloudStatus" Style="{StaticResource PrimaryButton}" Margin="0,0,0,8"/>
              <Button x:Name="BtnCloudOpen" Style="{StaticResource SecondaryButton}"/>
            </StackPanel>
          </Border>
        </Grid>
      ...
      <Border Grid.Row="3" Background="{DynamicResource PanelBrush}" BorderBrush="{DynamicResource LineBrush}" BorderThickness="1" CornerRadius="12" Padding="10" Margin="0,14,0,0" Visibility="Visible">
        <DockPanel>
...
function Start-NeoProcess {
    ...
    try {
        ...
        $wrappedCommand = "$consolePrelude; try { $CommandText } catch { Write-Error `$_; Start-Sleep -Seconds 4; exit 1 }; Start-Sleep -Seconds 2"
        $argumentText = "-NoProfile -ExecutionPolicy RemoteSigned -Command $(Quote-Arg $wrappedCommand)"
        $proc = Start-Process -FilePath (Get-PowerShellExe) -ArgumentList $argumentText -WorkingDirectory $Script:Root -WindowStyle Normal -PassThru
        Write-UiLog "Started action '$Label' as PID $($proc.Id)."
        Set-Status ("Console opened: {0}" -f $Label)
...
function Start-NeoAction {
    ...
    try {
        ...
        if ($Action -in @("SystemRepair", "WindowsErrorFix", "DiskRepair", "Services")) {
            if (-not (Confirm-NeoUiAction -Title "Confirm $Action" -Message "$Action can change Windows configuration. Continue in a visible console?")) {
        ...
        if ($Action -eq "AIPlan" -and (Invoke-RmmAiPlanDispatch)) {
            Update-SystemDashboard
            return
        }
        $rmmCfg = Read-RmmConfig
        if ($rmmCfg.dispatch_to_online_agents -and (Invoke-RmmDispatch $Action)) {
            Update-SystemDashboard
            return
        }
...
function Invoke-AiDoctor {
    ...
    try {
        ...
        $command = "& $(Quote-Arg $aiScript) -Mode Plan"
        if (Start-NeoProcess -Label "AI Doctor" -CommandText $command) {
            $processOwnsBusyState = $true
        }
    } finally {
        if (-not $processOwnsBusyState) {
...
function Read-ModelAgentConfig {
    $fallback = [PSCustomObject]@{
        ...
        ollama = [PSCustomObject]@{ endpoint = "http://127.0.0.1:11434/api/generate"; tags_endpoint = "http://127.0.0.1:11434/api/tags"; preferred_models = @("neo-light", "qwen2.5:3b-instruct", "llama3.2:3b") }
...
function Read-RmmConfig {
    $fallback = [PSCustomObject]@{
        ...
        candidate_server_urls = @("http://10.10.10.1:3000", "http://192.168.122.1:3000", "http://127.0.0.1:3000")
        auth = [PSCustomObject]@{ token = ""; email = ""; password = "" }
        dispatch_to_online_agents = $true
    }
    ...
    try {
        ...
        if ($null -eq $cfg.dispatch_to_online_agents) { $cfg | Add-Member -NotePropertyName dispatch_to_online_agents -NotePropertyValue $true -Force }
        return $cfg
    } catch {
        return $fallback
    }
...
function Get-RmmAuthToken {
    ...
    try {
        $login = Invoke-RmmJson -Url ($ServerUrl.TrimEnd("/") + "/api/v1/auth/login") -Method "Post" -Body @{ email = $email; password = $password } -TimeoutSec 5
...
function Get-RmmSnapshot {
    ...
    foreach ($url in $urls) {
        $base = $url.TrimEnd("/")
        try {
            $health = Invoke-RmmJson -Url ($base + "/health") -Method "Get" -TimeoutSec 2
            ...
            if (-not [string]::IsNullOrWhiteSpace($token)) {
                ...
                try {
                    $snapshot.stats = Invoke-RmmJson -Url ($base + "/api/v1/dashboard/stats") -Method "Get" -Token $token -TimeoutSec 5
                    $agentResponse = Invoke-RmmJson -Url ($base + "/api/v1/dashboard/agents?limit=50") -Method "Get" -Token $token -TimeoutSec 5
                    $commandResponse = Invoke-RmmJson -Url ($base + "/api/v1/dashboard/commands?limit=20") -Method "Get" -Token $token -TimeoutSec 5
...
function Update-AiProviderStatus {
    ...
    $statusText.Text = "$environmentName`r`nInteractive operator: enabled`r`nSkills: $skillCount | MCP connectors: $connectorCount`r`n$neoCoreStatus`r`n$ollamaStatus`r`n$nullclawStatus$extra`r`n$(T "AiRuleReady")."
}
...
function Show-AboutDialog {
    ...
    Add-AboutText "Open-Core Product Model" 16 "TextBrush" -Bold | Out-Null
    Add-AboutBox "Community Free" "Local optimizer, basic health checks, local-first AI guidance, and transparent safety documentation." "GoodBrush"
    Add-AboutBox "Pro Personal" "AI Doctor, scheduled maintenance, update assistant, richer reports, and safer one-click workflows for individual users." "AccentBrush"
    Add-AboutBox "RMM Team / Business" "Fleet dashboard, signed command dispatch, telemetry, Safety Auto-Response, update distribution, and endpoint policy control." "WarnBrush"
    Add-AboutBox "Enterprise Self-Hosted" "Private deployment, custom branding, compliance controls, support workflow, and advanced security guardrails." "DangerBrush"

    Add-AboutText "Support Development" 16 "TextBrush" -Bold | Out-Null
    Add-AboutText "Your support helps cover server, maintenance, Codex, internet, Hugging Face Spaces, Supabase, E2B, domain, website, and monthly infrastructure costs." 12 "MutedBrush" | Out-Null

    $linkPanel = [System.Windows.Controls.WrapPanel]::new()
    $linkPanel.Margin = [System.Windows.Thickness]::new(0, 4, 0, 12)
    $linkPanel.Children.Add((New-AboutButton "BuyMeCoffe" { Open-NeoExternalLink "https://buymeacoffee.com/nol.eight" })) | Out-Null
...
function Apply-Localization {
    ...
    Set-ControlContent "BtnAiOperator2" "NEO Operator"
    Set-ControlContent "BtnAiProviders2" (T "Providers")
...
function Update-SystemDashboard {
    ...
    if ($ForceRmm -or $null -eq $rmmSnapshot -or ((Get-Date) - $Script:LastRmmRefresh).TotalSeconds -ge 10) {
...
Add-ControlClick "BtnProfilePage" { Start-NeoAction "Profile" }
Add-ControlClick "BtnModelSettingsPage" { Invoke-NeoUiTask -Label "AI Model Settings" -Work { Show-AiModelSettings } }
Add-ControlClick "BtnUsers" { Show-NeoReportsPage -NavName "BtnUsers" }
...
$Window.Add_Loaded({
    try {
        ...
        Set-Status (T "Ready")
        Update-SystemDashboard -ForceRmm
